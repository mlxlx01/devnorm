...
import type { CliOptions, ReviewResult, Finding } from '../types.js';
...
function detectStack(files: string[]): string {
  ...
  const stack: string[] = [];
  if (hasTsConfig) stack.push('TypeScript (strict)');
  if (hasReact) stack.push('React');
  if (hasPackage) stack.push('Node.js');
  return stack.length > 0 ? stack.join(', ') : 'Unknown stack';
}

function buildReviewPrompt(stack: string, files: Array<{ path: string; content: string }>): string {
...
function parseReviewOutput(output: string): ReviewResult {
  ...
  if (stackMatch) result.stackDetected = stackMatch[1].trim();
  ...
  if (findingsMatch) {
    ...
    for (const block of findingBlocks) {
      ...
      const finding: Finding = {
        severity: titleMatch[1] as Finding['severity'],
        title: titleMatch[2],
        file: '',
        confidence: 8,
        failure: '',
      ...
      for (const line of lines) {
        if (line.startsWith('- File:')) finding.file = line.replace('- File:', '').trim();
        if (line.startsWith('- Confidence:')) finding.confidence = parseInt(line.replace('- Confidence:', '').trim());
        if (line.startsWith('- Failure:')) finding.failure = line.replace('- Failure:', '').trim();
        if (line.startsWith('- Fix:')) finding.fix = line.replace('- Fix:', '').trim();
      }
      if (finding.file && finding.failure) result.findings.push(finding);
    }
  }
